...
class SchedulerOutputProcessorMixin:
    ...
    def _update_reasoning_tokens_for_req(self: Scheduler, req: Req, token_id: int) -> None:
        sampling_params = getattr(req, "sampling_params", None)
        if sampling_params is None:
            return

        think_begin_id = getattr(sampling_params, "think_begin_str_id", None)
        think_end_id = getattr(sampling_params, "think_end_str_id", None)

        if think_begin_id is None or think_end_id is None:
            return

        if not req.in_reasoning_phase:
            if token_id == think_begin_id:
        ...
        if self.is_generation:
            ...
            for i, req in enumerate(batch.reqs):
                ...
                if req.is_chunked <= 0:
                    ...
                    self._update_reasoning_tokens_for_req(req, 0)
                    req.check_finished()
